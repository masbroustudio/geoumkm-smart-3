...
## N. Model Documentation Page

The dashboard includes a Model Documentation page accessible at `/model-docs`. This page provides:

- **Model Cards** in Google Model Card format for both the Location Scoring (XGBoost) and Credit Risk (Logistic Regression) models
- **Business Impact Summary** showing measurable outcomes for government, banking, and investor stakeholders
- **Data Pipeline Overview** listing what each notebook (NB01-NB08) produces
- **Model Limitations & Ethical Considerations** including synthetic data caveats and usage guidelines
- **Deployment Readiness Checklist** showing current status of production requirements

### Important Notes

- Model performance data originates from `ml/data/executive_summary.json` (generated by Notebook 08)
- The page displays static values hardcoded in the component
- If notebooks are re-run with different data, update the static values in:
  - `frontend/app/(dashboard)/model-docs/page.tsx` (model metrics, business impact numbers)
  - `frontend/lib/model-performance-data.ts` (model performance chart data)
  - `frontend/lib/dbscan-data.ts` (DBSCAN clustering metrics)
- The sidebar link uses the `BookOpen` icon from lucide-react
